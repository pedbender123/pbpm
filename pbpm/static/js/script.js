document.addEventListener('DOMContentLoaded', () => {
    ...
    // Adicionamos um IF para garantir que este código só execute se o chat existir na página
    ...
    if (chatModal) {
        ...
        chatModal.addEventListener('click', (e) => { if (e.target === chatModal) hideModal(); });

        window.addEventListener('scroll', () => {
            if (!servicesSection) return;
            const triggerPoint = servicesSection.offsetTop;
            if (window.scrollY > triggerPoint) {
                if (chatNotification) chatNotification.classList.remove('hidden');
                setTimeout(() => { if (chatNotification) chatNotification.classList.remove('opacity-0', 'translate-y-10'); }, 10);
            } else {
                if (chatNotification) chatNotification.classList.add('opacity-0', 'translate-y-10');
                setTimeout(() => { if (chatNotification) chatNotification.classList.add('hidden'); }, 500);
            }
        });
        ...
        addExternalMessage('ai', 'Olá! Sou o assistente da PBPM. Para começarmos, qual ideia de projeto você tem em mente?');
        ...
        chatForm.addEventListener('submit', async (e) => {
            ...
            try {
                const response = await fetch('/api/external_chat', {
                    ...
                    body: JSON.stringify({ message: userInput, count: externalMessageCount })
                });
                ...
                if (data.disable_chat) {
                    if(chatInputContainer) {
                        chatInputContainer.innerHTML = `<a href="/auth" class="w-full text-center bg-gradient-to-r from-purple-600 to-blue-500 text-white font-bold text-lg px-8 py-3 rounded-xl hover:opacity-90 transition-opacity">Registrar e Iniciar Projeto</a>`;
                    }
                }
            } catch (error) {
                console.error("Erro no chat externo:", error);
                addExternalMessage('ai', "Desculpe, nosso assistente parece estar offline. Tente novamente mais tarde.");
            }
